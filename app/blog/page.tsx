...
export default function BlogIndexPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-zinc-100 py-24 px-6 md:px-12 relative overflow-hidden font-sans">
      ...
      <div className="max-w-4xl mx-auto mb-12 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between border-b border-zinc-900 pb-8 select-none">
        ...
        {loading ? (
          ...
          filteredPosts.map((post, idx) => (
            <motion.div
              ...
              className="group bg-zinc-950 border border-zinc-900 rounded-2xl p-6 md:p-8 hover:border-emerald-500/30 transition-all duration-300 shadow-lg shadow-black/40 hover:shadow-emerald-500/2"
            >
              <div className="flex flex-col gap-4">
                ...
                <Link
                  href={`/blog/${post.slug}`}
                  className="text-xl md:text-2xl font-bold text-white group-hover:text-emerald-400 transition-colors duration-200"
                >
                  {post.title}
                </Link>
                ...
                <div className="flex items-center justify-between mt-4 border-t border-zinc-900 pt-5">
                  ...
                  <Link
                    href={`/blog/${post.slug}`}
                    className="flex items-center gap-1 text-xs font-mono font-bold uppercase tracking-wider text-zinc-400 group-hover:text-emerald-400 transition-colors cursor-pointer"
                  >
